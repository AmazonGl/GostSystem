...
import { storageApi, gostsApi, api, type Gost, type GostMetaSchema, type StorageNode } from '../../api/client'
import {
...
// Открыть файл ГОСТа в новой вкладке с передачей токена авторизации
async function previewGost(id: string, fileType?: string, fileName?: string) {
  try {
    const resp = await api.get(`/gosts/${id}/preview`, { responseType: 'blob', params: { _t: Date.now() } })
    ...
    // PDF браузер показывает во вкладке; остальные форматы (DOCX и пр.) — скачиваем
    if (mime.includes('pdf')) {
      window.open(url, '_blank', 'noopener')
    } else {
      const link = document.createElement('a')
      link.href = url
      link.download = fileName || `gost.${fileType || 'docx'}`
    ...
    toast.error('Не удалось открыть файл: ' + (e?.response?.data?.detail || e?.message || 'ошибка'))
  }
}
...
function ExpandableText({ text }: { text: string; limit?: number }) {
  const [open, setOpen] = useState(false)
  if (!text) return null
  return (
    <div className="pl-4 text-dim leading-relaxed">
              ...
              <div className="mt-2 space-y-1.5 pl-3 border-l-2 border-accent/20">
                {s.subsections!.map(sub => (
                  <div key={sub.id} className="text-xs space-y-0.5">
                    ...
                    {sub.description && (
                      <ExpandableText text={sub.description} limit={150} />
                    )}
                  </div>
                ))}
              </div>
      ...
      <div className="w-full max-w-lg bg-surface border-l border-border h-full overflow-y-auto flex flex-col">
        ...
        <div className="p-4 space-y-4 flex-1">
          <div className="flex gap-2">
            <button onClick={() => previewGost(gost.id, gost.file_type, `${gost.code || gost.title}.${gost.file_type}`)} className="btn-ghost text-xs">
    ...
    <div className="p-6 max-w-5xl">
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-dim">Загруженные стандарты ГОСТ · папки · просмотр</p>
        </div>
      ...
      {showUpload && (
        <form onSubmit={upload} className="card mb-6 space-y-4">
          ...
          <p className="text-xs text-dim">При загрузке автоматически разбирается структура разделов стандарта</p>
              ...
              <div key={g.id} className="flex items-center gap-3 p-2 rounded hover:bg-card group transition-colors">
                ...
                <div className="flex-1 min-w-0 cursor-pointer" onClick={() => setSelectedGost(g)}>
                  ...
                  <div className="text-xs text-dim font-mono">
                    ...
                    {g.has_template && <span className="text-accent ml-2">· шаблон ✓</span>}
                  </div>
                </div>
                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => previewGost(g.id, g.file_type, `${g.code || g.title}.${g.file_type}`)} className="btn-ghost text-xs px-2 py-1">
                    <Eye size={12} />
                  </button>
