...
import { Users, BookOpen, MessageSquare, FileText, Layers, Hash } from 'lucide-react'

interface Stats {
  counts: { users: number; gosts: number; sessions: number; docs: number; roadmaps: number; messages: number }
  top_gosts: { code: string; title: string; sessions: number }[]
  ...
  return (
    <div className="p-6 max-w-4xl">
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        ...
        <StatCard icon={Layers}        label="Роадмапов"     value={data.counts.roadmaps} />
        <StatCard icon={MessageSquare} label="Сессий"        value={data.counts.sessions} />
        <StatCard icon={Hash}          label="Сообщений"     value={data.counts.messages} />
        <StatCard icon={FileText}      label="Документов"    value={data.counts.docs} />
      </div>

      <div className="grid grid-cols-2 gap-4">
        <div className="card">
          <div className="text-xs font-mono text-dim uppercase tracking-widest mb-3">Топ ГОСТов по сессиям</div>
          {data.top_gosts.length === 0
            ...
            : data.top_gosts.map((g, i) => (
              <div key={i} className="flex items-center gap-3 py-2 border-b border-border last:border-0">
                ...
                <span className="tag">{g.sessions} сессий</span>
              </div>
            ))
          }
        </div>
