...
import { MessageSquare, FileText, BookOpen, Terminal, FolderOpen, Layers, LogOut, Shield, Cpu, Users, BarChart2, UserCircle, LayoutTemplate, PenLine } from 'lucide-react'
        ...
        <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
          ...
          {user?.role === 'admin' && (
            <>
              ...
              <NavItem to="/admin/storage"  icon={FolderOpen} label="ГОСТы / Хранилище" />
              <NavItem to="/admin/templates" icon={LayoutTemplate} label="Шаблоны" />
              <NavItem to="/admin/prompts"  icon={Terminal}   label="Промпты" />
              <NavItem to="/admin/roadmaps" icon={Layers}     label="Роадмапы" />
