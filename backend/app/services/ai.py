...
async def chat(messages: list[dict], system: str = "", temperature: float = 0.7) -> str:
    payload = {"model": MODEL, "messages": messages, "stream": False,
                "options": {"temperature": temperature, "seed": random.randint(1, 999999)}}
    if system:
...
def extract_sections_from_text(text: str) -> list[dict]:
    ...
    def _flush_sub():
        ...
        if current_sub:
            current_subsections.append({
                "num": current_sub[0],
                "title": current_sub[1],
                "content": ' '.join(current_sub_content[:3]),
            })
        current_sub = None
        current_sub_content = []
    ...
    def _flush_section():
        ...
        if current_section:
            sections.append({
                ...
                "content": ' '.join(current_content[:5]),
    ...
    for line in lines:
        ...
        if not line:
            continue
        main = _re.match(r'^(\d+)\s+([А-ЯA-Z][^\n]{5,60})$', line)
        if main:
            _flush_sub()
            _flush_section()
            current_section = (main.group(1), main.group(2).strip())
            continue
        sub = _re.match(r'^(\d+\.\d+(?:\.\d+)?)\s+([А-ЯA-Za-zа-я][^\n]{3,80})$', line)
        if sub and current_section:
            _flush_sub()
            current_sub = (sub.group(1), sub.group(2).strip())
            continue
        if current_sub and line and len(line) > 10:
            current_sub_content.append(line)
        elif current_section and line and len(line) > 20:
...
async def assistant_reply(user_message: str, system: str = "", history: list[dict] | None = None) -> str:
    ...
    try:
        return await chat(messages, system=system, temperature=0.6)
    except Exception:
        return (
